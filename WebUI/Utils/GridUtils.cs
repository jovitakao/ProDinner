namespace Omu.ProDinner.WebUI.Utils
{
    public static class GridUtils
    {
        ...
        {
            return string.Format("<button type='button' class='awe-btn awe-nonselect' onclick=\"awe.open('{0}', {{ params:{{ {1}: '.{1}' }} }})\"><span class='ico-edit'></span></button>",
                popupName, key);
        }
        ...
        {
            return string.Format("<button type='button' class='awe-btn {3}' onclick=\"awe.open('{0}', {{ params:{{ {1}: '.{1}' }} }})\">{2}</button>",
